...
function Overflow (container) {
  ...
  this.content = $(container)
    .find('.o-overflow__content')
    .get(0)
  this.count = $(this.content).children().length
  this.childrenWidth = parseInt(
    $(this.content)
      .children()
      .first()
      .width(),
  ...
  this.innerWidth = parseInt(
    this.count * this.childrenWidth + this.spaceBetween,
  )
  ...
  this.getShiftWidth = function (direction) {
    var shiftCount = Math.round(
      (this.outerWidth - overflowNavWidth) / this.childrenWidth,
    )
    var shiftWidth = this.childrenWidth * shiftCount
    ...
    switch (direction) {
      ...
      default:
        if (
          this.currentPos + shiftWidth + this.outerWidth <=
                    this.innerWidth
        ) {
          return shiftWidth
        } else {
